%% CONFIGURAÇÕES — QUALIDADE MÉDIA
images_dir = 'd:\uoal\images';        % pasta com todas as imagens
output_dir = 'd:\uoal\videos_medium'; % pasta de saída dos vídeos
fps        = 20;                       % FPS médio (entre 60 do ótimo e 8 do poor)
resolution = '1280x720';              % resolução entre Full HD e 640x360
crf        = 40;                       % qualidade entre 25 (ótimo) e 51 (poor)

% Tabela: usuário → ObjectIDs (em ordem)
users = {
    1, [166, 435, 101, 110,  21, 112,  72, 140,  23, 16, 29];
    2, [ 0, 36, 17, 20, 25, 27,  5, 68,  8, 19, 64];
    3, [ 4, 38, 21, 18, 26, 28, 33, 72, 65, 31    ];
    4, [ 6, 39, 37, 30, 82, 54, 88, 90             ];
    5, [ 7, 40, 42, 44, 46, 48, 15, 23             ];
};
%% CRIA PASTA DE SAÍDA
...
for i = 1:size(users, 1)
    ...
    tmp_dir = fullfile(tempdir, sprintf('user%d_frames_medium', user_id));
